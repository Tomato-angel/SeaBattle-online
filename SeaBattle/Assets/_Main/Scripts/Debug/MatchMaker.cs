...
public class MatchMaker
{
    ...
    {
        Debug.Log($"-- Starts disconnect game with key: {key} the Player: {player.connectionToClient.connectionId}");
        if (_matches.ContainsKey(key))
        {
            if (_matches[key].ContainsPlayer(player))
            {
                _matches[key].DeletePlayer(player);
    ...
    // Изменить все сигнатуры методов void на bool
    // Реализовать некоторый PlayerNotifier который будет перелавать на клиент данные об ошибке с сервера о его операции
